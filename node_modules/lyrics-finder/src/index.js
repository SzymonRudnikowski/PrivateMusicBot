...
async function main(e = "", d = "") {
    let i;
    try {
        i = await fetch(`${url}${encodeURIComponent(d + " " + e)}+lyrics`, headers);
        ...
        console.log(m);
        try {
            i = await fetch(`${url}${encodeURIComponent(d + " " + e)}+song+lyrics`, headers);
            i = await i.textConverted();
            [, i] = i.split(delim1);
            [i] = i.split(delim2);
        } catch (n) {
            try {
                i = await fetch(`${url}${encodeURIComponent(d + " " + e)}+song`, headers);
                i = await i.textConverted();
                [, i] = i.split(delim1);
                [i] = i.split(delim2);
            } catch (o) {
                try {
                    i = await fetch(`${url}${encodeURIComponent(d + " " + e)}`, headers);
                    i = await i.textConverted();
                    [, i] = i.split(delim1);
                    [i] = i.split(delim2);
                } catch (p) {
