...
module.exports = {
  ...
  async execute(message, args, com, client) {
    ...
    if (!playlistName.length)
      return message.reply(' **you have to specify the name of the playlist you want to play**');
    if (!play_type.length)
      return message.reply(
        ' **you have to specify the way of playing your playlist (repeat/shuffle)**'
      );
    if (play_type !== 'shuffle' && play_type !== 'repeat' && play_type !== 'none')
      return message.reply(` **there is no such an option (${play_type})**`);
          ...
          serverLocal[guildID].forEach((playlist) => {
            if (playlist.name == playlistName) {
              switch (play_type) {
                case 'shuffle': {
                  if (
                    !songTitles.has(message.guild.id) ||
                    songTitles.get(message.guild.id).length === 1
                  ) {
                  }
                  break;
                }
