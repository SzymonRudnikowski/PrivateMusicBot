...
module.exports = {
    ...
    async execute(message, args, com, client) {
        ...
        if (!playlistName.length) return message.reply(' **you have to specify the name of the playlist you want a song to add in**');
        if (!song_name.length) return message.reply(' **you have to specify the name of the song you want to add in**');
                    ...
                    fs.writeFile(`./jsons_playlists/${message.guild.id}.json`, return_string, err => {
                        if (err) {
                            ...
                            const messEmbediss = new Discord.MessageEmbed()
                                .setTitle(`**👍** ***${song.title}*** **added to playlist** ***${playlistName}***`).setColor('PURPLE')
