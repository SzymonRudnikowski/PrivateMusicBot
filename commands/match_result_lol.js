...
module.exports = {
	...
	async execute(message, args, com, client) {
		...
		if (!team1Found && !team2Found) {
			...
			const messEmbednow = new MessageEmbed().setTitle(`**Team** ***${team1}*** **does not exist!**`).setColor('RED').setTimestamp();
			return message.channel.send(messEmbednow);
		} else if (!team2Found) {
			const messEmbednow = new MessageEmbed().setTitle(`**Team** ***${team2}*** **does not exist!**`).setColor('RED').setTimestamp();
			return message.channel.send(messEmbednow);
		}
