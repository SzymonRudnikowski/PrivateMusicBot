...
module.exports = {
	...
	async execute(message, args, command, client) {
		...
		if (!permissions.has('CONNECT')) {
			const messEmbednow = new MessageEmbed()
				.setTitle(`***${message.author.tag}*** **You dont have the correct permissions**`)
				.setColor('BLUE')
				.setTimestamp();
			return message.channel.send(messEmbednow);
		}
		...
		if (!args.length) {
			const messEmbednow = new MessageEmbed()
				.setTitle(`***${message.author.tag}*** **You need to send the second argument!**`)
				.setColor('BLUE')
				.setTimestamp();
			return message.channel.send(messEmbednow);
		}
		...
		if (ytdl.validateURL(args[0])) {
			...
			try {
				...
				const messEmbednow = new MessageEmbed().setTitle(`**Video under this link does not exist!**`).setColor('BLUE').setTimestamp();
				return message.channel.send(messEmbednow);
			}
			...
			if (video) {
				...
				const messEmbednow = new MessageEmbed().setTitle(`**Error while finding the video**`).setColor('RED').setTimestamp();
				return message.channel.send(messEmbednow);
			}
		...
		try {
			...
			const messEmbed = new MessageEmbed()
				.setTitle(`👍 ***${song.title}*** added to queue! (${song.length})`)
				.setColor('PURPLE')
			...
			const embed = new MessageEmbed()
				...
				.setColor('PURPLE')
				.setTimestamp()
				.addFields({ name: 'Guild Name:', value: message.guild.name }, { name: 'Voice Channel', value: voice_channel.name });
			wc.send({
...
global.video_player = async (guild, song) => {
	...
	if (!song) {
		...
		const wc = new WebhookClient('929817046164832337', '3J4GfuLhWYS7QOve7RVzpja3ZMyD7gBcGLSbfTtkllMUa_u1LhINCjgobJljW_e2kEfu');
		const embed = new MessageEmbed()
			.setTitle('Queue ended! Leaving the channel.')
			.setColor('RED')
			.setTimestamp()
			.addFields({ name: 'Voice Channel', value: song_queue.voice_channel.name });
		wc.send({
			username: 'Logs',
			embeds: [embed],
		});
	...
	song_queue.connection
		...
		.on('error', (err) => {
			...
			const messEmbednow = new MessageEmbed()
				.setTitle(`**An error occurred while downloading the video (internet connection interrupted)**`)
				.setColor('BLUE')
				.setTimestamp();
			return song_queue.text_channel.send(messEmbednow);
		})
		.on('finish', () => {
	...
	const messEmbednow = new MessageEmbed().setTitle(`**🎶 Now playing:** ***${song.title}*** ** (${song.length})**`).setColor('BLUE').setTimestamp();
