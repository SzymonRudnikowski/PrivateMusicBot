  ...
  async execute(message, args, command, client) {
    ...
    if (ytdl.validateURL(args[0])) {
      ...
      try {
        ...
        const messEmbednow = new MessageEmbed()
          .setTitle(`**Video under this link does not exist!**`)
          .setColor('BLUE')
          .setTimestamp();
        return message.channel.send(messEmbednow);
      }
      ...
      if (len_sec >= 3600) {
        if (len_sec / 60 / 60 < 10)
          video_length_formatted += '0' + Math.floor(len_sec / 60 / 60) + ':';
      ...
      if (video) {
        ...
        const messEmbednow = new MessageEmbed()
          .setTitle(`**Error while finding the video**`)
          .setColor('RED')
          .setTimestamp();
        return message.channel.send(messEmbednow);
      }
...
global.video_player = async (guild, song) => {
  ...
  song_queue.connection
    ...
    .on('error', (err) => {
      ...
      const messEmbednow = new MessageEmbed()
        .setTitle(
          `**An error occurred while downloading the video (internet connection interrupted)**`
        )
        .setColor('BLUE')
        .setTimestamp();
  ...
  const messEmbednow = new MessageEmbed()
    .setTitle(`**🎶 Now playing:** ***${song.title}*** ** (${song.length})**`)
    .setColor('BLUE')
    .setTimestamp();
