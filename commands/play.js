...
module.exports = {
    ...
    async execute(message, args, command, client) {
        ...
        if (!permissions.has('CONNECT')) {
            const messEmbednow = new MessageEmbed()
                .setTitle(`***${message.author}*** **You dont have the correct permissions**`).setColor('BLUE').setTimestamp();
            return message.channel.send(messEmbednow);
        }
        ...
        if (!args.length) {
            const messEmbednow = new MessageEmbed()
                .setTitle(`***${message.author}*** **You need to send the second argument!**`).setColor('BLUE').setTimestamp();
            return message.channel.send(messEmbednow);
        }
...
global.video_player = async (guild, song) => {
    ...
    song_queue.connection.play(stream, { seek: 0, volume: 0.5 })
        .on('error', err => {
            ...
            const messEmbednow = new MessageEmbed()
                .setTitle(`**An error occurred while downloading the video (internet connection interrupted)**`).setColor('BLUE').setTimestamp();
