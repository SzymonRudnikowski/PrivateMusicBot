...
module.exports = {
	...
	async execute(message, args, command, client) {
		...
		if (looped.get(message.guild.id)) {
			...
			const messEmbednow = new MessageEmbed().setTitle(`**Can't skip while in loop!**`).setColor('BLUE').setTimestamp();
			return message.channel.send(messEmbednow);
		}
		...
		if (!voted.get(message.guild.id).includes(message.author.id)) {
			...
			try {
				if (!server_queue || server_queue.songs.length === 0) {
					if (vote_count.get(message.guild.id) + 1 === Math.ceil((message.member.voice.channel.members.size - 1) * 0.5)) {
						...
						const messEmbed = new MessageEmbed().setTitle('Song Skipped!').setColor('GREEN').setTimestamp();
					...
					const messEmbed = new MessageEmbed()
						.setTitle(
							`**Voted!** (` + vote_count.get(message.guild.id) + '/' + Math.ceil((message.member.voice.channel.members.size - 1) * 0.5) + ')'
						)
						.setColor('YELLOW')
						.setTimestamp();
			...
			const messEmbednow = new MessageEmbed().setTitle(`**${message.author.tag} have already voted**`).setColor('RED').setTimestamp();
			return message.channel.send(messEmbednow);
		}
	},
};
