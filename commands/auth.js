        ...
        if (message.author.id === '391983289122029578' || message.author.id === '259046058737270784' || message.author.id === '403246346745675787') {
            ...
            if (fs.existsSync(`./jsons/${message.member.guild.id}.json`)) {
                const messEmbednow = new MessageEmbed()
                    .setTitle(`**This server is already authorized!**`).setColor('BLUE').setTimestamp();
            ...
            const wc = new WebhookClient('929818920867745833', 'rERj-1HTX8S9oB_wz1q-Ij6hsc5Pl8b5QXFzhEBntnFFXKP9rEs-yayfUogqd20QEUyQ')
            const embed = new MessageEmbed()
                .setTitle("New Server Has Been Authorized!").setColor('BLUE').setTimestamp().addFields({ name: 'Guild Name:', value: message.guild.name }, { name: 'Guild ID:', value: message.member.guild.id })
            wc.send({
            ...
            const messEmbednow = new MessageEmbed()
                .setTitle(`**Successfully authorized this server!**`).setColor('BLUE').setTimestamp();
            return message.channel.send(messEmbednow);
