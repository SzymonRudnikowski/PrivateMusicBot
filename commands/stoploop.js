...
module.exports = {
	...
	async execute(message, args, com, client) {
		...
		if (!looped.get(message.guild.id)) {
			const messEmbednow = new MessageEmbed().setTitle(`****Nothing is looped!**`).setColor('BLUE').setTimestamp();
			return message.channel.send(messEmbednow);
		}
		looped.set(message.guild.id, false);
		console.log('unlooped using stoploop! status: ' + looped.get(message.guild.id));
		const messEmbednow = new MessageEmbed().setTitle(`**Loop ended!**`).setColor('BLUE').setTimestamp();
		return message.channel.send(messEmbednow);
	},
};
