...
module.exports = {
    ...
    async execute(message, args, com, client) {
        ...
        let song_name = args.join().substr(args.join().indexOf('/') + 1, args.join().length).replace(/,/g, ' ');
        console.log(song_name);

        let playlistName = args.join().substr(0, args.join().indexOf('/')).replace(/,/g, ' ');
        console.log(playlistName);
        if (!playlistName.length) return message.reply(' **you have to specify the name of the playlist you want a song to remove**');
        if (!song_name.length) return message.reply(' **you have to specify the name of the song you want to remove**');
                ...
                if (err) {
                    ...
                    if (!song_exist) {
                        const messEmbediss = new Discord.MessageEmbed()
                            .setTitle(`**Song** ***${song_name}*** **does not exist in playlist** ***${playlistName}***`).setColor('PURPLE')
                    ...
                    fs.writeFile(`./jsons_playlists/${message.guild.id}.json`, return_string, err => {
                        if (err) {
                            ...
                            const messEmbediss = new Discord.MessageEmbed()
                                .setTitle(`**👍** ***${song_name}*** **deleted from playlist** ***${playlistName}***`).setColor('PURPLE')
