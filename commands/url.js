            ...
            setTimeout(async () => {
                if (!exist) {
                    ...
                    if (!right_players.get(message.guild.id)) {
                        const messEmbednow = new MessageEmbed()
                            .setTitle(`**The number of players who are not participating in the tournament cannot exceed 1 per team! \n \n If someone from your team has changed their nickname during MLE or in the registration process has given a nickname that differs from their original Faceit nickname please inform admins about it - otherwise, you won't be able to upload stats for your game!**`).setColor('RED').setTimestamp();
                        return message.channel.send(messEmbednow);
                    }
