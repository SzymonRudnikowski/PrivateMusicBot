...
module.exports = {
  ...
  async execute(message, args, command, client) {
    ...
    try {
      ...
      if (queue_constructor.songs.length === 1 || queue.get(message.guild.id).songs.length === 1)
        text.set(message.guild.id, 'Empty');
      else {
        ...
        for (let i = 1; i <= 10; i++) {
          ...
          text.set(
            message.guild.id,
            text.get(message.guild.id) +
              i +
              '. ' +
              queue.get(message.guild.id).songs[i].title +
              '\n'
          );
        }
      }
