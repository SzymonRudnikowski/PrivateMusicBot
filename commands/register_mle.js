...
module.exports = {
	...
	async execute(message, args, com, client) {
		...
		setTimeout(() => {
			if (!idFree) {
				const messEmbednow = new MessageEmbed().setTitle(`**This id has already been used!**`).setColor('RED').setTimestamp();
				return message.channel.send(messEmbednow);
			}
			...
			member
				...
				.catch((err) => {
					console.log(err);
					const messEmbednow = new MessageEmbed()
						.setTitle(`**There was an error during registration process. Please contact <@&391983289122029578>**`)
						.setColor('RED')
						.setTimestamp();
					return message.channel.send(messEmbednow);
				});
			...
			const messEmbednow = new MessageEmbed().setTitle(`**Successfully registered! Category: ${categoryRole}**`).setColor('GREEN').setTimestamp();
			return message.channel.send(messEmbednow);
		}, 100);
	},
};
