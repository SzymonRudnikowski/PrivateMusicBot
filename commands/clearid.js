  ...
  async execute(message, args, com, client) {
    ...
    setTimeout(() => {
      if (idFree) {
        const messEmbednow = new MessageEmbed()
          .setTitle(
            `**This user has not yet registered for this category! Category: (${categoryRole})**`
          )
          .setColor('RED')
          .setTimestamp();
        return message.channel.send(messEmbednow);
      ...
      member.roles.remove(mainRole).catch((err) => {
        ...
        const messEmbednow = new MessageEmbed()
          .setTitle(
            `**There was an error during user deletion process. Please contact <@391983289122029578>**`
          )
          .setColor('RED')
          .setTimestamp();
        return message.channel.send(messEmbednow);
