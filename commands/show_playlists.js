    ...
    if (!args || !args.join().length) {
      ...
      if (!playlistName.length)
        return message.reply(
          ' **you need to specify the name of the playlist you want to see the content of!**'
        );
      ...
      setTimeout(() => {
        if (!exist)
          return message.channel.send(
            `**Playlist named** ***${playlistName}*** **does not exist!**`
          );
                ...
                if (playlist.name === playlistName) {
                  if (!playlist.songs.length) page_text = 'Empty';
                    ...
                    playlist.songs.forEach((song) => {
                      ...
                      if (len_sec >= 3600) {
                        if (len_sec / 60 / 60 < 10)
                          song_length_formatted += '0' + Math.floor(len_sec / 60 / 60) + ':';
                      ...
                      if (len_sec % 60 < 10)
                        song_length_formatted += '0' + Math.floor(len_sec % 60);
