	...
	async execute(message, args, com, client) {
		...
		if (!args || !args[0].length) {
			const messEmbednow = new MessageEmbed()
				.setTitle(`***${message.author}*** **you have to specify the user you want to ban!**`)
				.setColor('BLUE')
				.setTimestamp();
		...
		if (!person.bannable) {
			const messEmbednow = new MessageEmbed()
				.setTitle(`**I do not have permissions to ban** ***${person.user.tag}***`)
				.setColor('BLUE')
				.setTimestamp();
			return message.channel.send(messEmbednow);
		}
		...
		try {
			...
			if (targetId) {
				...
				const messEmbednow = new MessageEmbed().setTitle(`***${person.user.tag}*** ** is already banned!**`).setColor('BLUE').setTimestamp();
				return message.channel.send(messEmbednow);
			}
		} catch (err) {
			console.log(`${person.user.tag} not banned yet`);
			if (!message.guild.member(userId)) {
				const messEmbednow = new MessageEmbed().setTitle(`**There is no user named** ***${person.user.tag}***`).setColor('BLUE').setTimestamp();
				return message.channel.send(messEmbednow);
			}
		}
		...
		if (1) {
			if (message.mentions.members.first()) {
				...
				const messEmbednow = new MessageEmbed()
					.setTitle(`***${person.user.tag}*** **has been successfuly banned from this server!**`)
					.setColor('BLUE')
					.setTimestamp();
				return message.channel.send(messEmbednow);
			}
			...
			const messEmbednow = new MessageEmbed()
				.setTitle(`***${person.user.tag}*** **You do not have permissions to ban** ***${person.user.tag}***`)
				.setColor('BLUE')
				.setTimestamp();
			return message.channel.send(messEmbednow);
		}
