  ...
  async execute(message, args, com, client) {
    ...
    try {
      ...
      if (targetId) {
        ...
        const messEmbednow = new MessageEmbed()
          .setTitle(`***${person.user.tag}*** ** is already banned!**`)
          .setColor('BLUE')
          .setTimestamp();
        return message.channel.send(messEmbednow);
      }
      ...
      if (!message.guild.member(userId)) {
        const messEmbednow = new MessageEmbed()
          .setTitle(`**There is no user named** ***${person.user.tag}***`)
          .setColor('BLUE')
          .setTimestamp();
        return message.channel.send(messEmbednow);
      }
    ...
    if (message.member.hasPermission(['ADMINISTRATOR'])) {
      ...
      const messEmbednow = new MessageEmbed()
        .setTitle(
          `***${person.user.tag}*** **You do not have permissions to ban** ***${person.user.tag}***`
        )
        .setColor('BLUE')
        .setTimestamp();
      return message.channel.send(messEmbednow);
