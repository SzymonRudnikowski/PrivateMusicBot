...
module.exports = {
	...
	async execute(message, args, com, client) {
		...
		if (hasJoinedChannel.has(message.guild.id)) {
			const messEmbednow = new MessageEmbed().setTitle(`**I'm already in a channel!**`).setColor('BLUE').setTimestamp();
		...
		try {
			...
			const messEmbednow = new MessageEmbed().setTitle(`**There was an error connecting!**`).setColor('BLUE').setTimestamp();
			message.channel.send(messEmbednow);
