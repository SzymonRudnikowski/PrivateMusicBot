...
module.exports = {
    ...
    async execute(message) {
        ...
        try {
            ...
            const messEmbednow = new MessageEmbed()
                .setTitle(`**There is nothing to clear - the queue is empty!**`).setColor('BLUE').setTimestamp();
            return message.channel.send(messEmbednow);
        }

        const messEmbednow = new MessageEmbed()
            .setTitle(`**Queue has been successfully cleared!**`).setColor('BLUE').setTimestamp();
        return message.channel.send(messEmbednow);
    }
}
